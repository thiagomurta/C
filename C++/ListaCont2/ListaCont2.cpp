...
void ListaCont2::set(int k, int val)  ///altera valor na posicao k
...
ListaCont2* ListaCont2::copiar(int val){
    int j = -1;
    for(int i=inicio; i<= ultimo(); i++){
        if(vet[i]==val){
            j = i;
    ...
    ListaCont2* p = new ListaCont2(max);
    if(j!=-1){
...
///QUESTÃO 5.3
void ListaCont2::inserirVet(int tam, int v[]){
    max = max + tam;
...
///QUESTÃO 5.6
void ListaCont2::intercala(ListaCont2 *la, ListaCont2 *lb){
    ///Limpar a lista
    delete [] this->vet;
    ///
    n = 0;
...
bool ListaCont2::troca(int posicao1 , int posicao2 ){
    int aux;
...
bool ListaCont2::aumentaCapacidade(int novoMax){
    if(novoMax>max){
        max = novoMax;
        int *auxVet = new int[max];
...
int ListaCont2::numNos(){
    return n;
}

void ListaCont2::limpar(){
    n=0;
}
int ListaCont2::buscaMaior(int val){
    int i;
    for(i=0;i<n;i++){
        if(vet[i]>val){
            return i;
